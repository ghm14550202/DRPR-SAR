...
Fig. 3 shows the overall framework of DRPR-SAR. The method consists of two stages. In the first stage, a task-guided decoupling module (TGDM), implemented with a rotation-enhanced VQ-VAE, decomposes each SAR image into a redundant stream and a discriminative stream. In the second stage, adversarial fine-tuning introduces dynamic perturbation routing, which encourages attack-induced variations to concentrate in the discriminative stream, while knowledge distillation guides the redundant-stream classifier to preserve class semantics learned from a clean teacher model.
