...
meanstd_dataset = {
    # 'svhn': [[0.0, 0.0, 0.0],
    # [1.0, 1.0, 1.0]],
    'mstar': [[0.13620707], [0.11149936]],
    'acd': [[0.0565084], [0.09828296]],
    'opensar': [[0.05527896], [0.06979666]],
    'FUSAR': [[0.10994489], [0.10608266]]
...
class Normalize(nn.Module):
    def __init__(self, dataset='FUSAR', input_channels=1):
        super(Normalize, self).__init__()
        # self.input_size = imgsize_dataset[dataset.lower()]
        self.input_size = imgsize_dataset[dataset]
        self.input_channels = input_channels

        # self.mean, self.std = meanstd_dataset[dataset.lower()]
        self.mean, self.std = meanstd_dataset[dataset]
...
class Self_Attn(nn.Module):
    ...
    def __init__(self, in_channels, spectral_norm):
        ...
        if spectral_norm:
            ...
            self.conv1x1_phi = snconv2d(in_channels=in_channels, out_channels=in_channels // 8, kernel_size=1, stride=1,
                                        padding=0, bias=False)
            self.conv1x1_g = snconv2d(in_channels=in_channels, out_channels=in_channels // 2, kernel_size=1, stride=1,
                                      padding=0, bias=False)
            self.conv1x1_attn = snconv2d(in_channels=in_channels // 2, out_channels=in_channels, kernel_size=1,
                                         stride=1, padding=0, bias=False)
        else:
            self.conv1x1_theta = conv2d(in_channels=in_channels, out_channels=in_channels // 8, kernel_size=1, stride=1,
                                        padding=0, bias=False)
            self.conv1x1_phi = conv2d(in_channels=in_channels, out_channels=in_channels // 8, kernel_size=1, stride=1,
                                      padding=0, bias=False)
            self.conv1x1_g = conv2d(in_channels=in_channels, out_channels=in_channels // 2, kernel_size=1, stride=1,
                                    padding=0, bias=False)
            self.conv1x1_attn = conv2d(in_channels=in_channels // 2, out_channels=in_channels, kernel_size=1, stride=1,
                                       padding=0, bias=False)
...
class RandBatchNorm2d(nn.Module):
    ...
    def forward(self, input):
        ...
        out = F.batch_norm(input, self.running_mean, self.running_var, weight, bias,
                           self.training or not self.track_running_stats, exponential_average_factor, self.eps)
