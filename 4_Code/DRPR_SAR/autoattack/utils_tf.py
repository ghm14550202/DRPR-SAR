...
class ModelAdapter():
    def __init__(self, logits, x, y, sess, num_classes=10):#num_classes=10
        self.logits = logits
        self.sess = sess
        self.x_input = x
        self.y_input = y
...
def dlr_loss(x, y, num_classes=10):#num_classes=10
...
def dlr_loss_targeted(x, y, y_target, num_classes=10):#num_classes=10
    x_sort = tf.contrib.framework.sort(x, axis=1)
    y_onehot = tf.one_hot(y, num_classes)
